...
#define DEFAULT_SUBNET_MASK_ADDR 0x00FFFFFF
...
int lwip_option_dump(int len, char *param[])
{
    opl_lwip_mode_t mode;
    opl_lwip_option_t option;
    opl_lwip_option_get(&mode, &option);
    OPL_LWIP_OPTION_DUMP(mode, option);
    return 0;
}
...
int lwip_cli_ip_cfg(int len, char *param[])
{
    ...
    uint32_t ip_addr, ip_sub_addr, ip_gate_addr;
    
    if (mode > STA_IP_MODE_DHCP) {
        printf("Invalid dhcp config \r\n");
        return -1;
    }
    
    if (mode == STA_IP_MODE_STATIC) {
        if (len < 2) {
        ...
        if (len >= 2) {
            ip_addr = ipaddr_addr(param[1]);
            ip_sub_addr = DEFAULT_SUBNET_MASK_ADDR;
            ip_gate_addr = (ip_addr & DEFAULT_SUBNET_MASK_ADDR) | (1 << 24);
        }
        
        if (len >= 3) {
            ip_sub_addr = ipaddr_addr(param[2]);
        }
        
        if (len >= 4) {
            ip_gate_addr = ipaddr_addr(param[3]);
        }
    }
    
    printf("Current Setting \r\n");
    printf("    DHCP = %d (0:STATIC, 1:DHCP) \r\n", mode);
    if (mode == STA_IP_MODE_STATIC) {
        printf("    IP      = 0x%08x \r\n", ip_addr);
    ...
    if (mode == STA_IP_MODE_STATIC) { 
        dhcp_config_set_mode(STA_IP_MODE_STATIC);
        netif_set_addr(&netif, (ip4_addr_t*) &ip_addr, (ip4_addr_t*) &ip_sub_addr, (ip4_addr_t*) &ip_gate_addr);
        lwip_net_start(WIFI_MODE_STA_ONLY);
    }
    else { //DHCP
        dhcp_config_set_mode(STA_IP_MODE_DHCP);
        lwip_net_start(WIFI_MODE_STA_ONLY);
...
int lwip_cli_arp_dup_addr_det(int len, char *param[])
{
extern u8_t g_lwip_arp_duplicate_addr_check;
    int on_off = atoi(param[0]);
    if (on_off == 1) {
        g_lwip_arp_duplicate_addr_check = 1;
        printf("ARP duplicate address detect enabled.\r\n");
    }
    else if (on_off == 0) {
        g_lwip_arp_duplicate_addr_check = 0;
        printf("ARP duplicate address detect disabled.\r\n");
    }
    return 0;
}
#endif
...
int lwip_cli_dhcp_restore(int len, char *param[])
{
extern u8_t g_do_dhcp_restore;
    int on_off = atoi(param[0]);
    if (on_off == 1) {
        g_do_dhcp_restore = 1;
        printf("DHCP restore the previous IP address.\r\n");
    }
    else if (on_off == 0) {
        g_do_dhcp_restore = 0;
        printf("DHCP not restore the previous IP address.\r\n");
    }
    return 0;
}
#endif
...
cli_command_t lwip_cli[] = {
    ...
    { "option",     "show lwip options",    lwip_option_dump,               NULL },
    { "perf",       "lwip build-in perf",   lwip_cli_lwperf,                NULL },
    { "ip",         "IP config",            lwip_cli_ip_cfg,                NULL },
#if OPL_ARP_DUPLICATE_ADDR_DETECT
    { "arp_detect",  "ARP duplicate address detect",     lwip_cli_arp_dup_addr_det, NULL},
#endif
#if OPL_DHCP_RESTORE_LAST_IP
    { "dhcp_restore","Restore the previous IP address.", lwip_cli_dhcp_restore,     NULL},
#endif
