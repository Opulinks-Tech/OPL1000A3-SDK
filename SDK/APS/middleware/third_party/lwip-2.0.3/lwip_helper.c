...
static void lwip_check_timeouts(PS_WAKEUP_TYPE wake_type)
{
    ...
    wakeup_event_timeouts = wakeup_event_timeouts_max;
...
static int32_t wifi_station_disconnected_event_handler_impl(void *arg)
{
    ...
    printf("[lwip_helper] wifi disconnected\r\n");
    return 1;
}
...
/**
  ...
  * @retval None
  */
static int32_t wifi_station_connect_failed_event_handler_impl(void *arg)
{
    LWIP_UNUSED_ARG(arg);
    if(dhcp_config_init() == STA_IP_MODE_DHCP) {
        lwip_net_stop(WIFI_MODE_STA_ONLY);
...
RET_DATA wifi_station_connect_failed_event_handler_fp_t wifi_station_connect_failed_event_handler;
...
void lwip_load_interface_lwip_helper(void)
{
    ...
    wifi_station_connect_failed_event_handler = wifi_station_connect_failed_event_handler_impl;
