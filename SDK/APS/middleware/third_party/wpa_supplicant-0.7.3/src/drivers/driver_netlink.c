...
int wpa_driver_netlink_reset_impl(void)
{
    ctrl_wifi_reset();
    return WPA_DRIVER_OK;
}

int wpa_driver_netlink_scan_impl(int mode)
{
    if (wifi_ctrl_state_get() == WIFI_CTRL_SCANNING_ST ||
        wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_SCAN_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore scan, state in scanning.\r\n");
        return WPA_DRIVER_IN_SCANNING;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTING_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore scan, state in connecting.\r\n");
        return WPA_DRIVER_IN_CONNECTING;
    }
...
Boolean wpa_driver_netlink_scan_stop_impl(void)
{
    ...
    else if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_SCAN_ST) {
        wifi_scan_stop_req();
    }
...
char *get_ssid_by_bsssid_impl(char *pbssid)
{
    int i;
...
int wpa_driver_cal_psk_impl(char *pssid, uint16_t ssid_len, char *ppassphrase, uint16_t pass_len)
{
    if (pssid == NULL)
        return -1;
...
int wpa_driver_netlink_connect_impl(struct wpa_config * conf)
{
...
int wpa_driver_netlink_connect_by_bssid_impl(u8 *bssid)
{
    int ret = 0;
	if (bssid == NULL) return WPA_DRIVER_INVALID_PARAM;
    ...
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore connect, state in connected.\r\n");
        return WPA_DRIVER_IN_CONNECTED;
    }
    
    ret = wifi_sta_join(bssid);
    wpa_supplicant_set_state(wpa_s, WPA_ASSOCIATING);
    if(ret == 0) return WPA_DRIVER_OK;
    return WPA_DRIVER_OK;
}

int wpa_driver_netlink_connect_from_ac_impl(u8 index)
{
    if (index > MAX_NUM_OF_AUTO_CONNECT) {
        log_print(LOG_HIGH_LEVEL, TAG, "WPA: Invalid Parameter \r\n");
        return WPA_DRIVER_INVALID_PARAM;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_SCANNING_ST ||
        wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_SCAN_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore connect from ac, state in scanning.\r\n");
        return WPA_DRIVER_IN_SCANNING;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTING_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore connect from ac, state in connecting.\r\n");
        return WPA_DRIVER_IN_CONNECTING;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore connect from ac, state in connected.\r\n");
        return WPA_DRIVER_IN_CONNECTED;
    }
...
int wpa_driver_netlink_reconnect_impl(struct wpa_config * conf)
{
    int ret = 0;
	if (conf == NULL) return WPA_DRIVER_INVALID_PARAM;
    if (conf->ssid == NULL) return WPA_DRIVER_INVALID_PARAM;
    ...
    if (wifi_ctrl_state_get() == WIFI_CTRL_SCANNING_ST ||
        wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_SCAN_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore reconnect, state in scanning.\r\n");
        return WPA_DRIVER_IN_SCANNING;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTING_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore reconnect, state in connecting.\r\n");
        return WPA_DRIVER_IN_CONNECTING;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore reconnect, state in connected.\r\n");
        return WPA_DRIVER_IN_CONNECTED;
    }
...
int wpa_driver_netlink_disconnect_impl(const u8 *bssid, u16 reason_code)
{
    if (wifi_ctrl_state_get() == WIFI_CTRL_SCANNING_ST ||
        wifi_ctrl_state_get() == WIFI_CTRL_CONNECTED_SCAN_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore disconnect, state in scanning.\r\n");
        return WPA_DRIVER_IN_SCANNING;
    }
    
    if (wifi_ctrl_state_get() == WIFI_CTRL_CONNECTING_ST) {
        log_print(LOG_MED_LEVEL, TAG, "ignore disconnect, state in connecting.\r\n");
        return WPA_DRIVER_IN_CONNECTING;
    }
...
int wpa_driver_netlink_sta_cfg_impl(u8 mode, u8 cmd_idx, void *value)
{  
    ...
    switch (mode) {
        case MLME_CMD_GET_PARAM:
            wifi_get_sta_cfg_req(cmd_idx, value);
            break;
...
int wpa_driver_netlink_scan_by_cfg_impl(void *cfg)
{
    if (cfg == NULL) return WPA_DRIVER_INVALID_PARAM;
...
RET_DATA wpa_driver_netlink_reset_fp_t wpa_driver_netlink_reset;
...
RET_DATA wpa_driver_netlink_fast_connect_set_fp_t wpa_driver_netlink_fast_connect_set;
RET_DATA wpa_driver_netlink_sta_cfg_fp_t wpa_driver_netlink_sta_cfg;
RET_DATA wpa_driver_netlink_get_state_fp_t wpa_driver_netlink_get_state;
...
RET_DATA wpa_driver_netlink_set_encrypt_mode_fp_t wpa_driver_netlink_set_encrypt_mode;
RET_DATA wpa_driver_cal_psk_fp_t wpa_driver_cal_psk;
RET_DATA get_ssid_by_bsssid_fp_t get_ssid_by_bsssid;
...
void wpa_driver_func_init(void)
{
    ...
    wpa_driver_netlink_reset               = wpa_driver_netlink_reset_impl;
    ...
    wpa_driver_netlink_fast_connect_set    = wpa_driver_netlink_fast_connect_set_impl;
    wpa_driver_netlink_sta_cfg             = wpa_driver_netlink_sta_cfg_impl;
    wpa_driver_netlink_get_state           = wpa_driver_netlink_get_state_impl;
    ...
    wpa_driver_netlink_set_encrypt_mode    = wpa_driver_netlink_set_encrypt_mode_impl;
    wpa_driver_cal_psk                     = wpa_driver_cal_psk_impl;
    get_ssid_by_bsssid                     = get_ssid_by_bsssid_impl;
}
