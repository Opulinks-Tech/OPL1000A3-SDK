...
typedef enum {   
    ...
    E_WIFI_PARAM_BCN_TRACK_BOOST,
    E_WIFI_PARAM_BCN_TRACK_PLUS,
...
typedef struct {
    ...
    void *sta_cfg_semaphore;
    int (*sta_cfg_cb)(void *ptr);
} ctrl_wifi_unlock_cb_t;

typedef struct {
    uint16_t config_time;
    ctrl_wifi_unlock_cb_t cb;
} ctrl_wifi_cmd_tout_t;

typedef struct {
    uint8_t   u8InvlDiv;          /* SDTIM interval divisor. Default: 2 */
    uint8_t   u8GaTtlTime;        /* BC/MC listening window limit, unit: ms. Default: 20 */
    uint16_t  u16SkipBMcastInvl;  /* Skip BC/MC indication interval, unit: TIME UNIT. Default: 6500 */
} S_WIFI_BTP_PARAM;

typedef struct {
    uint8_t  sta_mac_addr[MAC_ADDR_LEN];
    uint8_t  sta_skip_dtim;
    uint32_t sta_tx_data_rate;
    uint16_t sta_bcn_track_boost;
    S_WIFI_BTP_PARAM btp;
} ctrl_wifi_m0_config_t;

typedef struct {
    ctrl_wifi_m0_config_t  m0_config;
    ctrl_wifi_cmd_tout_t   cmd_tout;
} ctrl_wifi_config_t;
...
int wifi_sta_get_scan_idx_impl(u8 *pBssid);
...
void control_auto_connect_run_first_impl (void);

int wifi_get_sta_cfg_req_impl(u8 idx, void *value);
int wifi_set_sta_cfg_req_impl(u8 idx, void *value);
...
void wifi_ctrl_create_scan_cmd_timer_impl(void);
int ctrl_wifi_send_to_supplicant_impl(uint32_t event, uint8_t *param);
int ctrl_wifi_send_to_event_loop_impl(uint32_t event, uint8_t *param);
int ctrl_wifi_cmd_tout_cb_init_impl(void);
int ctrl_wifi_cmd_config_cb_impl(void *ptr);
int ctrl_wifi_reset_impl(void);
int wifi_disconnect_cleanup_etharp_impl(void);
...
typedef int (*wifi_sta_get_scan_idx_fp_t)(u8 *pBssid);
...
typedef void (*hidden_ap_init_fp_t) (void);
...
typedef void (*control_auto_connect_run_first_fp_t) (void);

typedef int (*wifi_get_sta_cfg_req_fp_t) (u8 idx, void *value);
typedef int (*wifi_set_sta_cfg_req_fp_t) (u8 idx, void *value);
...
typedef void (*wifi_ctrl_create_scan_cmd_timer_fp_t)(void);
typedef int (*ctrl_wifi_send_to_supplicant_fp_t)(uint32_t event, uint8_t *param);
typedef int (*ctrl_wifi_send_to_event_loop_fp_t)(uint32_t event, uint8_t *param);
typedef int (*ctrl_wifi_cmd_tout_cb_init_fp_t)(void);
typedef int (*ctrl_wifi_cmd_config_cb_fp_t)(void *prt);
typedef int (*ctrl_wifi_reset_fp_t)(void);
typedef int (*wifi_disconnect_cleanup_etharp_fp_t)(void);
...
extern control_auto_connect_run_first_fp_t control_auto_connect_run_first;

extern wifi_get_sta_cfg_req_fp_t wifi_get_sta_cfg_req;
...
extern wifi_ctrl_create_scan_cmd_timer_fp_t wifi_ctrl_create_scan_cmd_timer;
extern ctrl_wifi_send_to_supplicant_fp_t ctrl_wifi_send_to_supplicant;
extern ctrl_wifi_send_to_event_loop_fp_t ctrl_wifi_send_to_event_loop;
extern ctrl_wifi_cmd_tout_cb_init_fp_t ctrl_wifi_cmd_tout_cb_init;
extern ctrl_wifi_cmd_config_cb_fp_t ctrl_wifi_cmd_config_cb;
extern ctrl_wifi_reset_fp_t ctrl_wifi_reset;
extern wifi_disconnect_cleanup_etharp_fp_t wifi_disconnect_cleanup_etharp;
...
extern hidden_ap_init_fp_t hidden_ap_init;
