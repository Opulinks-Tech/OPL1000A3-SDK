...
#define IPC_SPARE0_MSQ_FLASH_READY   (1 << 6)       /* Removed */
#define IPC_SPARE0_APS_CLOCK_READY   (1 << 7)
#define IPC_SPARE0_VOL_CHECK_DIS     (1 << 8)   // Disable voltage checking
#define IPC_SPARE0_VOL_CHECK_FAIL    (1 << 9)   // Valtage checking failed
...
#define IPC_WIFI_CMD_DATA_SIZE      12 //IPC_VALUE_ALIGN(sizeof(wifi_cmd_t), 4)
#define IPC_WIFI_EVT_DATA_SIZE      12 //IPC_VALUE_ALIGN(sizeof(wifi_evt_t), 4)
...
#else
#define IPC_WIFI_MSQ_TX_BUF_SIZE        1120 //IPC_VALUE_ALIGN(sizeof(wifi_mac_msq_tx_entry_t) + 1, 16)
#endif
...
#else
#define IPC_WIFI_MSQ_RX_BUF_SIZE        1200 //IPC_VALUE_ALIGN(sizeof(wifi_mac_msq_rx_entry_t) + 1, 16)
#endif
...
#else
#define IPC_WIFI_APS_TX_BUF_SIZE        688 //IPC_VALUE_ALIGN(sizeof(wifi_mac_aps_tx_entry_t) + 1, 16)
#endif

#define IPC_WIFI_APS_TX_BUF_NUM         16 //IPC_DATA_BUF_NUM
...
#else
#define IPC_WIFI_APS_RX_BUF_SIZE        1696 //IPC_VALUE_ALIGN(sizeof(wifi_mac_aps_rx_entry_t) + 1, 16)
#endif
...
#define IPC_WIFI_APS_TX_BUF_LEN         (g_ptIpcCb[IPC_TYPE_WIFI_APS_TX].dwBufSize)
#define IPC_WIFI_APS_RX_BUF_LEN         (g_ptIpcCb[IPC_TYPE_WIFI_APS_RX].dwBufSize)
#define IPC_WIFI_MSQ_TX_BUF_LEN         (g_ptIpcCb[IPC_TYPE_WIFI_MSQ_TX].dwBufSize)
#define IPC_WIFI_MSQ_RX_BUF_LEN         (g_ptIpcCb[IPC_TYPE_WIFI_MSQ_RX].dwBufSize)
/*
 * IPC ring buffer length = FRAME_LEN + trx_frame_info + tx_entry_head and align to 16-byte
 * The FRAME_LEN includes MAC_HEADER and Frame body
 */
#define IPC_WIFI_APS_TX_FRAME_LEN       (g_ptIpcCb[IPC_TYPE_WIFI_APS_TX].dwBufSize - 64)
#define IPC_WIFI_APS_RX_FRAME_LEN       (g_ptIpcCb[IPC_TYPE_WIFI_APS_RX].dwBufSize - 128)
#define IPC_WIFI_MSQ_TX_FRAME_LEN       (g_ptIpcCb[IPC_TYPE_WIFI_MSQ_TX].dwBufSize - 64)
#define IPC_WIFI_MSQ_RX_FRAME_LEN       (g_ptIpcCb[IPC_TYPE_WIFI_MSQ_RX].dwBufSize - 128)
...
#define IPC_M0_MSG_BUF_SIZE             20 //IPC_VALUE_ALIGN(IPC_M0_MSG_SIZE + 1, 4)
...
#define IPC_M3_MSG_BUF_SIZE             20 //IPC_VALUE_ALIGN(IPC_M3_MSG_SIZE + 1, 4)
...
typedef struct
{
    ...
    uint32_t *pdwWrite;
    uint32_t *pdwRead;
    uint32_t *pdwaBuf[32]; // >= IPC_WIFI_APS_TX_BUF_NUM
} T_IpcWifiApsTxRb;
/*
typedef struct
{
...
typedef void *(*T_IpcBufGetByIdxFp)(uint32_t u32Idx);
typedef int (*T_IpcBufGetDoneFp)(uint32_t u32Num);
...
extern T_IpcWifiCountGetFp ipc_aps_tx_rb_num_get;
...
extern T_IpcBufGetByIdxFp ipc_aps_tx_read_buf_get_by_idx;
extern T_IpcBufGetDoneFp ipc_aps_tx_read_buf_get_done;
#else
...
extern T_IpcRbReadBufGetFp ipc_rb_write_buf_get;
extern T_IpcBufGetByIdxFp ipc_aps_tx_write_buf_get_by_idx;
extern T_IpcBufGetDoneFp ipc_aps_tx_write_buf_get_done;
...
void Ipc_RbPreInit(uint32_t u32RbAddrStart);
