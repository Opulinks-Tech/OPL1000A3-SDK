...
// 0x3F.18 Vendor-Specific Get Packet Info Statistics
typedef struct
{
    char *log_buf_addr;

} Vendor_Specific_Get_Packet_Info_Statistics_Command_Return_Parameters;
...
typedef struct
{
    ...
    // MISC
    uint8_t                     is_request_reset:1;
    uint8_t                     display_info_en:1;                  // display packet info whenever BLE is disconnected
    
	// debug message switches
	uint8_t 					open_dbg_hci_standard:1;		    // open all standard hci command/event log
	uint8_t 					open_dbg_ll_hci_hook:1;				// open all link layer internal handler log
	uint8_t 					open_dbg_ll_enc:1;					// open all encryption related log
    
    // LE gain control
    uint8_t                     agc_rf_gain_level;
    uint8_t                     agc_digital_gain_level;
