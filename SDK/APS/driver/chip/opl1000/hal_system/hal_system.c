...
RET_DATA T_Hal_Sys_ApsPclkGet      Hal_Sys_ApsPclkGet;
...
uint32_t Hal_Sys_RcCal_impl(void)
{
    ...
    {
        // Disable
        RF->RC_OSC_CAL_CTL &= ~RC_CAL_EN;
        
        // Reset, 0:reset 
        RF->RC_OSC_CAL_CTL &= ~RC_CAL_RST;
        RF->RC_OSC_CAL_CTL |= RC_CAL_RST;
        ...
        while( 1 )
        {
            u32Xtal = (RF->RC_OSC_CAL_OUTPUT) & RC_CAL_XTAL_CNT_MSK;
            if( u32Xtal >= 10000)
            {
                // Disable
                RF->RC_OSC_CAL_CTL &= ~RC_CAL_EN;
        ...
        // Get RC ticks
        u32Rc = ( (RF->RC_OSC_CAL_OUTPUT) & RC_CAL_RC_CNT_MSK ) >> RC_CAL_RC_CNT_POS;
 ...
 */
uint32_t Hal_Sys_ApsPclkGet_impl(void)
{
    /* If PCLK divn enabled, PCLK = SYSCLK/2 */
    return SystemCoreClockGet() >> (g_u8PclkDivEn_Curr && 1);
}
...
void Hal_Sys_Pre_Init(void)
{
    ...
    Hal_Sys_ApsPclkGet      = Hal_Sys_ApsPclkGet_impl;
