...
T_Uart_RxCallBack            Uart_0_RxCallBack;
T_Uart_RxCallBack            Uart_1_RxCallBack;
T_HalUartConfig              g_taHalUartCurrentConfig[UART_IDX_MAX];
S_Uart_Reg_t *g_Uart_psaUartReg[UART_IDX_MAX] = {UART_0, UART_1};

T_Hal_Uart_RxIntEn           Hal_Uart_RxIntEn;
T_Hal_Uart_IntIdRead         Hal_Uart_IntIdRead;
T_Hal_Uart_IntClear          Hal_Uart_IntClear;
T_Hal_Uart_BaudRateSet       Hal_Uart_BaudRateSet;
T_Hal_Uart_Init              Hal_Uart_Init;
T_Hal_Uart_ConfigGet         Hal_Uart_ConfigGet;
T_Hal_Uart_ConfigSet         Hal_Uart_ConfigSet;
T_Hal_Uart_RxCallBackFuncSet Hal_Uart_RxCallBackFuncSet;

// Those function are used for general case, time-out with default value(1 ms)
T_Hal_Uart_DataSend Hal_Uart_DataSend;
T_Hal_Uart_DataRecv Hal_Uart_DataRecv;

// Those functions are recommand used only for bootROM and ISR, time-out with the given value
T_Hal_Uart_DataSendTimeOut Hal_Uart_DataSendTimeOut;
T_Hal_Uart_DataRecvTimeOut Hal_Uart_DataRecvTimeOut;
...
uint32_t Hal_Uart_BaudRateSet_impl(E_UartIdx_t eUartIdx, uint32_t u32Baud)
{
    ...
    {   /* Invalid divider, set to default baud rate */
        u32Ret = 1;
        u32Div = ( (UART_SRC_CLK >> 4) + (DW_UART_DEFAULT_BAUD >> 2) ) / DW_UART_DEFAULT_BAUD;
    }
